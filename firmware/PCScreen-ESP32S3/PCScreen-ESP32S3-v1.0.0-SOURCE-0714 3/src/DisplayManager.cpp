...
void drawVietnameseHeader(lgfx::LGFX_Sprite& canvas, uint8_t page,
  ...
  else if (page == 3) bitmap = &ViText::kPerformance;
  canvas.drawXBitmap(12, 7, bitmap->data, bitmap->width, bitmap->height,
                     color);
}
}  // namespace
...
void DisplayManager::render(const TelemetryData& data, bool connected,
  ...
  header(language_ == UiSettings::Language::English ? titles[currentPage_] : "",
         connected, light);
  if (language_ == UiSettings::Language::Vietnamese) {
    drawVietnameseHeader(canvas_, currentPage_, foreground(light));
  }
