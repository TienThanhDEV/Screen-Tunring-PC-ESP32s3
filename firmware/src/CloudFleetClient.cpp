...
extern const uint8_t kRootCaBundleStart[]
...
void CloudFleetClient::loop() {
  if (checking_ || updating_ || WiFi.status() != WL_CONNECTED ||
      !clockReady()) return;
...
bool CloudFleetClient::checkNow() {
  ...
  if (!clockReady()) {
    serverReachable_ = false;
    lastError_ = "clock_not_synchronized";
    return false;
  }
...
void CloudFleetClient::reportTelemetry(const TelemetryData& telemetry,
  ...
  if (!isnan(telemetry.diskLoad)) values["diskLoad"] = telemetry.diskLoad;
  if (!isnan(telemetry.networkDownMbps)) values["networkDownMbps"] = telemetry.networkDownMbps;
  if (!isnan(telemetry.networkUpMbps)) values["networkUpMbps"] = telemetry.networkUpMbps;
  if (!isnan(telemetry.loadAverage)) values["loadAverage"] = telemetry.loadAverage;
  values["uptimeSeconds"] = telemetry.systemUptimeSeconds;
  values["processCount"] = telemetry.processCount;
  String body;
  serializeJson(document, body);

  WiFiClientSecure tls;
  tls.setCACertBundle(kRootCaBundleStart);
  HTTPClient http;
