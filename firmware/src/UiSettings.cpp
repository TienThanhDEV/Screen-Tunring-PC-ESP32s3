...
bool UiSettings::begin() {
  ...
  fontScalePercent_ = constrain(preferences_.getUChar("font_scale", 100), 85, 115);
  headerBold_ = preferences_.getBool("bold_header", true);
  labelBold_ = preferences_.getBool("bold_label", true);
  valueBold_ = preferences_.getBool("bold_value", true);
  customTextColors_ = preferences_.getBool("text_custom", false);
  primaryTextColor_ = preferences_.getUShort("text_main", 0xFFFF);
  secondaryTextColor_ = preferences_.getUShort("text_muted", 0x9CF3);
  telemetryIntervalMs_ = constrain(
      preferences_.getUShort("telemetry_ms", AppConfig::TELEMETRY_INTERVAL_DEFAULT_MS),
      AppConfig::TELEMETRY_INTERVAL_MIN_MS,
      AppConfig::TELEMETRY_INTERVAL_MAX_MS);
  telemetryPaused_ = preferences_.getBool("telemetry_off", false);
  const uint8_t transition = preferences_.getUChar("transition", 1);
  transitionStyle_ = transition <= 2
                         ? static_cast<TransitionStyle>(transition)
                         : TransitionStyle::Slide;
  transitionDurationMs_ = constrain(
      preferences_.getUShort("transition_ms", 420), 180, 900);
  smoothValues_ = preferences_.getBool("smooth_values", true);
  return true;
}
...
UiSettings::State UiSettings::state() const {
  return {theme_, language_, rotation_, autoRotate_, pageIntervalSeconds_, pageMask_,
          ...
          showDate_, cardRadius_, cardColors_, pageOrder_, fontScalePercent_,
          headerBold_, labelBold_, valueBold_, customTextColors_,
          primaryTextColor_, secondaryTextColor_, telemetryIntervalMs_,
          telemetryPaused_, transitionStyle_, transitionDurationMs_,
          smoothValues_};
}
...
void UiSettings::setFontScale(uint8_t percent) {
  fontScalePercent_ = constrain(percent, 85, 115);
  markDirty();
}

void UiSettings::setHeaderBold(bool enabled) { headerBold_ = enabled; markDirty(); }
void UiSettings::setLabelBold(bool enabled) { labelBold_ = enabled; markDirty(); }
void UiSettings::setValueBold(bool enabled) { valueBold_ = enabled; markDirty(); }
void UiSettings::setCustomTextColors(bool enabled) { customTextColors_ = enabled; markDirty(); }
void UiSettings::setPrimaryTextColor(uint16_t color) { primaryTextColor_ = color; markDirty(); }
void UiSettings::setSecondaryTextColor(uint16_t color) { secondaryTextColor_ = color; markDirty(); }

void UiSettings::setTelemetryInterval(uint16_t milliseconds) {
  telemetryIntervalMs_ = constrain(milliseconds,
                                   AppConfig::TELEMETRY_INTERVAL_MIN_MS,
                                   AppConfig::TELEMETRY_INTERVAL_MAX_MS);
  markDirty();
}

void UiSettings::setTelemetryPaused(bool paused) {
  telemetryPaused_ = paused;
  markDirty();
}

void UiSettings::setTransitionStyle(TransitionStyle style) {
  transitionStyle_ = style;
  markDirty();
}

void UiSettings::setTransitionDuration(uint16_t milliseconds) {
  transitionDurationMs_ = constrain(milliseconds, 180, 900);
  markDirty();
}

void UiSettings::setSmoothValues(bool enabled) {
  smoothValues_ = enabled;
  markDirty();
}
...
void UiSettings::save() {
  ...
  preferences_.putUChar("font_scale", fontScalePercent_);
  preferences_.putBool("bold_header", headerBold_);
  preferences_.putBool("bold_label", labelBold_);
  preferences_.putBool("bold_value", valueBold_);
  preferences_.putBool("text_custom", customTextColors_);
  preferences_.putUShort("text_main", primaryTextColor_);
  preferences_.putUShort("text_muted", secondaryTextColor_);
  preferences_.putUShort("telemetry_ms", telemetryIntervalMs_);
  preferences_.putBool("telemetry_off", telemetryPaused_);
  preferences_.putUChar("transition", static_cast<uint8_t>(transitionStyle_));
  preferences_.putUShort("transition_ms", transitionDurationMs_);
  preferences_.putBool("smooth_values", smoothValues_);
  dirty_ = false;
}
